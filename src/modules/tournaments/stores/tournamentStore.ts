...
export const useTournamentsStore = defineStore('tournaments', {
  ...
  actions: {
    async addTournament(newTournament: TournamentRequestDto) {
      const apiResponse = await http.post('/tournament', newTournament);
    ...
    async updateTournament(id: number, currentTournament: TournamentRequestDto) {
      const apiResponse = await http.put(`/tournament/${id}`, currentTournament);
      let tournamentsState = this.tournaments.filter((tournament) => tournament.id !== id);
      tournamentsState.push(apiResponse.data);
      this.tournaments = tournamentsState;
    },
    async removeTournament(id: number) {
      await http.delete(`/tournament/${id}`);
